...
public class AuthenticatedUserMapper implements MyMapper<AuthenticatedUser, AuthenticatedUserResponseDTO, AuthenticatedUserRequestDTO> {

    @Override
    public AuthenticatedUser toEntity(AuthenticatedUserRequestDTO dto) {
        
        return new AuthenticatedUser(dto.getFirstName(), dto.getLastName(), dto.getEmail(), dto.getPassword(), dto.getCompanyName(), null);
    }

    @Override
    public AuthenticatedUserResponseDTO toDto(AuthenticatedUser entity) {
        
        return new AuthenticatedUserResponseDTO(entity.getId(), entity.getFirstName(), entity.getLastName(), entity.getEmail(), entity.getCompanyName());
    }

    @Override
    public List<AuthenticatedUserResponseDTO> toDtoList(List<AuthenticatedUser> entityList) {
        
        List<AuthenticatedUserResponseDTO> authenticatedUserDTOs = new ArrayList<>();

        for (AuthenticatedUser authenticatedUser : entityList) {
            authenticatedUserDTOs.add(toDto(authenticatedUser));
        }
